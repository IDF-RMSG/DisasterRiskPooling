...
The graphs help us to think now about how we might structure the pool so that we can be as efficient as we  can be with the funding available. This is where we bring in Layer 3 - Risk transfer. What Risk transfer or Insurance can help us do is ensure we have access to funding for the biggest losses or payouts from layer 2, without having to hold it in layer 2.

It is very difficult to justify in any context holding onto a very large amount of money in case a rare event (with a small probability of occurring) happens, when there are needs in so many other places. But we still want and need to ensure that if those events do occur, we have access to enough funding (though it is unlikely that enough funding for all events will be in place). Instead, we can pay a small amount each year (a 'premium') for someone else to hold enough funding for us to cover those events through insurance. Insurance is set up best if covering rare events with low probabilities and managing its financial risk across much larger diverse global pools of risks. 

The more risk is transferred and taken on by an insurance company, the higher the premium. So it becomes less cost effective to transfer risk with lower return periods (or higher probability of loss) and with cinreasing coverage amount. The most uninsurable risk would be a high payout that happens frequentl, but these losses can be covered with layer 1 and 2 -  these layers and instruments should be financially and operationally efficient to ensure impact and predictability, but don't need to be profitable for insurance markets.
...
The risk pool layer 2 allows us to share the risks and probabilities of loss (payout) across the different country risks in the tool. However, insurance is able to share the transferred risk across many different kinds of global risks and probabilities (not only the ones that are being covered by your organisation). This means you don’t have to hold onto such a large amount of money to cover the low chance of needing it all in a given year, and it ensures we have coverage if those rarer events do happen. 
